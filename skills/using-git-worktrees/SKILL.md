...
### 2a. Skip if dependencies are symlinked (native worktree path)
...
**If dependency directories exist as symlinks:** Skip to Step 3. Running install commands against symlinked `node_modules` is unnecessary and may break the symlink or cause duplicate work.

**If no symlinks found, or Step 1b was used** (manual git worktree): Proceed with 2b.
...
| Symlinked `node_modules` (Step 1a) | Skip install, go to Step 3 |
...
- **Fix:** Step 2a checks `[ -L "node_modules" ]` before installing. If symlinked, skip.
