...
source_hooks="$(python3 - "$REPO_ROOT/.codex-plugin/plugin.json" <<'PY'
import json
import sys

with open(sys.argv[1], encoding="utf-8") as handle:
    print(json.load(handle).get("hooks"))
PY
)"
...
expected_version="$(python3 - "$REPO_ROOT/.codex-plugin/plugin.json" <<'PY'
import json
import sys
