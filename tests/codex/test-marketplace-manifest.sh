...
assert_equal(marketplace.get("name"), "my-superpowers", "marketplace name")
assert_equal(
...
assert_equal(
    plugin.get("source"),
    {"source": "url", "url": "https://github.com/THQDDQHT/superpowers.git", "ref": "main"},
    "plugin source",
)
assert_equal(
    plugin.get("policy"),
...
# marketplace installs the whole repo root, so on Codex the fallback
# re-registers the SessionStart hook and its install-time trust prompt.
